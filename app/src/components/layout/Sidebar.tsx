...
const ITEM_BASE  = "w-full flex items-center gap-2 px-3 py-2 text-sm font-medium transition-colors";
const SUBLINK    = "flex items-center gap-2 pl-10 pr-3 py-1 text-xs font-medium transition-colors";
...
export function Sidebar() {
  ...
  return (
    <>
      ...
      <aside
        className={[
          "no-print fixed top-14 left-0 z-20 h-[calc(100vh-3.5rem)] w-72 bg-white border-r border-gray-200",
      ...
      >
        <nav className="py-1">
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              [ITEM_BASE, isActive ? ACTIVE_CLS : IDLE_CLS].join(" ")
            }
          >
            <span>🏠</span>
            <span>Inicio</span>
            ...
            return (
              <div key={item.to}>
                <button
                  ...
                  className={[ITEM_BASE, "justify-between", isCurrentPath ? ACTIVE_CLS : IDLE_CLS].join(" ")}
                >
                  <span className="flex items-center gap-2 min-w-0 flex-1">
                    <span className="shrink-0">{item.icon}</span>
                    <span className="truncate">{item.label}</span>
                    <span className="text-[10px] text-gray-400 shrink-0 font-normal">{item.unitat}</span>
                  </span>
                  <span className="shrink-0 text-gray-400 text-xs">{isOpen ? "▾" : "▸"}</span>
                </button>

                {isOpen && (
                  <div className="bg-gray-50 border-l-2 border-indigo-100 ml-3">
